...
Goal: finish up Chapter 1 of MIL.
...
example (a b : ℝ) : (a + b)^3 = a^3 + 3*a^2*b + 3*a*b^2 + b^3 := by sorry

example (a b c : ℝ) : c + a + b + a - c = 2 • a + b := by sorry

example : 65 * 3 = 195 := by sorry

section

variable (R : Type) [CommRing R]

example (a b : R) : (a + b) * (a - b) = a^2 - b^2 := by sorry

example (a b : R) : (a + b)^3 = a^3 + 3*a^2*b + 3*a*b^2 + b^3 := by sorry

example (a b c : R) : c + a + b + a - c = 2 • a + b := by sorry

end

example (A : Type*) [AddCommGroup A] (a b c : A) : c + a + b + a - c = 2 • a + b := by sorry
