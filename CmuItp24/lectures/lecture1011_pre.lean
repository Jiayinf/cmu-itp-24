...
example : ({0, 1, 2} : Finset ℕ) = {1, 2, 0} := by sorry

example : ({0, 1, 2} : Finset ℕ) = {0, 1, 1, 2} := by sorry

example : ({0, 1} : Finset ℕ) = {1, 0} := by sorry
...
example : (range 10).filter Even = {0, 2, 4, 6, 8} := by sorry

#check (range 5).image (fun x ↦ x * 2)

example : (range 5).image (fun x ↦ x * 2) = (range 10).filter Even := by sorry
